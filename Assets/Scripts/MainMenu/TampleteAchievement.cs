...
public class TampleteAchievement : MonoBehaviour, IPointerClickHandler
{
    ...
    private void Start()
    {
        ...
        foreach (var item in Achievement.count)
        {
            ...
            {
                goa.LockIcon.color = Achievement.rewardSkin.color;
                
                //goa.Reward.color = Achievement.rewardSkin.color;
                s = "SKIN";
            }
            ...
            goa.LockIcon.gameObject.SetActive(!(i < Achievement.rewardsCoins.Count));
            i++;
        }
    }
